function brick-create-repo([string]$name) {
	Read-Host "API currently not cooperating, so go create the repo Intermark/$name (press enter when done)"
...
function brick-push-repo([string]$name) {
	cd $name
	git remote add origin git@github.com:Intermark/$name.git
